document.addEventListener('DOMContentLoaded', function() {
    ...
    // Observa quando PAG12 fica ativa
    ...
    function handleMouseMove(e) {
        if (!pag12.classList.contains('active')) return;
    ...
    function startShaking() {
        ...
        shakeInterval = setInterval(() => {
            ...
            pag12.style.transform = `translate(${x}px, ${y}px) rotate(${rotation}deg)`;
        }, 50);
    }

    function stopShaking() {
        clearInterval(shakeInterval);
        pag12.style.transform = '';
        isShaking = false;
    }

    function goToNextPage() {
        // Simula clique no botão next page
        const btnNext = document.getElementById('btn-pag12');
        if (btnNext) {
            btnNext.click();
        }
    }
